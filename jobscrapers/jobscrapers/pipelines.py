...
# # pipelines.py — thay thế toàn bộ 3 dict cũ
...
#     "6" : "Tiến sĩ",
#     "11": "Khác",
#     "12": "Cao đẳng",   # ← 12 = Cao đẳng, không phải "Không yêu cầu"
#     "-1": "Không xác định",
# }
...
#     #     item["job_deadline"] = ""
#     elif website == "linkedin":
#     # Nếu ai_processor đã điền job_description → giữ nguyên
#     # Nếu chưa → fallback về raw_about_job
#         if not item["job_description"]:
#             raw_desc = (raw.get("raw_about_job") or "").strip()
#             item["job_description"] = _clean_nbsp(raw_desc)

#         # job_requirement: giữ nếu AI đã điền, không xóa như cũ
#         # item["job_requirement"] = ""  ← XÓA dòng này

#         # job_posted_at: relative → dd/mm/yyyy (giữ nguyên logic cũ)
#         posted = item["job_posted_at"]
#         if _LI_TIME_PAT.search(posted):
#             item["job_posted_at"] = _relative_to_date(posted, scraped_iso)
...
#         raw_about_job    LONGTEXT,     -- Dành cho LinkedIn, lưu tạm trước khi chuyển vào job_description
...
# =========================================================
#  save_to_db() — dùng cho Selenium spider
#
#  Trả về:
#    (True,  "new")      → INSERT thành công, job hoàn toàn mới
#    (True,  "updated")  → ON DUPLICATE KEY UPDATE đã chạy (rowcount == 2)
#    (False, "duplicate")→ Không có gì thay đổi (rowcount == 0)
#    (False, "invalid")  → item không hợp lệ
#    (False, "error")    → MySQL lỗi
#
#  Giải thích rowcount với ON DUPLICATE KEY UPDATE (MySQL spec):
#    1 → INSERT mới
#    2 → UPDATE row cũ (duplicate key, có thay đổi)
#    0 → UPDATE row cũ nhưng values giống hệt (không thay đổi gì)
# =========================================================
